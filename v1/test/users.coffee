...
describe 'Users |', ->
  ...
  it 'fails to create user with already existing username', (done) ->
    ...
    .end (err, body) ->
      ...
      expect(body).to.be.an 'object'
      expect(body.success).to.equal false
      expect(body.error).to.equal 'A user with this username already exists'
      done()

  it 'fails to create user with already existing email', (done) ->
    dupEmail =
      username: 'dupEmail'
      password: 'bwgdb'
      role: 'admin'
    ...
    .end (err, body) ->
      ...
      expect(body).to.be.an 'object'
      expect(body.success).to.equal false
      expect(body.error).to.equal 'A user with this email already exists'
      done()
  ...
  describe 'Clean up user |', ->
    it 'should delete all users in this test suite', (done) ->
      api('users/delete')
      .method('POST')
